...
def readInput():
	...
	with open(file_dataset, mode='U',  encoding='utf-8-sig') as csv_file,  open(new_file_location, mode='a+', encoding='utf-8-sig') as csv_output:
		fieldnames = ['Vehicle_No', 'Date', 'Time', 'Location', 'Lat', 'Lng', 'Speed', 'Vehicle_Status', 'GPS', 'Distance-KM', 'Vehicle_Batt.-V', 'Route']
		...
		for row in csv_reader:
			...
			output = {'Vehicle_No':row["Vehicle_No"], 'Date':row["Date"], 'Time':row["Time"], 'Location':row["Location"], 'Lat':coordinate_dict['lat'], 'Lng':coordinate_dict['lng'], 'Speed':row["Speed"], 'Vehicle_Status':row["Vehicle_Status"], 'GPS':row["GPS"], 'Distance-KM':row["Distance-KM"], 'Vehicle_Batt.-V':row["Vehicle_Batt.-V"],'Route' : row["Route"]}
...
gmaps = googlemaps.Client(key='AIzaSyCjSZvrFkma3e4V655kmpIWm_jKr_BJI7o')
